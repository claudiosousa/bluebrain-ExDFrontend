...
(function () {
  ...
  angular.module('performanceMonitorModule')
      ...
      function () {
        function PerformanceMonitorService() {
          ...
          this.processStateChange = function(message) {
            if (angular.isDefined(message.brainsimElapsedTime) &&
              ...
              _.forEach(Object.keys(message.transferFunctionsElapsedTime), function(key) {
                if (index >= data.length) {
                  data.push(message.transferFunctionsElapsedTime[key]);
                  background.push(backgroundColors[index % backgroundColors.length]);
                  label.push(key);
                } else {
                  label[index] = key;
