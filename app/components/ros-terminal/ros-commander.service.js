...
(function() {
  ...
  class RosCommanderService {
    constructor(roslib, simulationInfo) {
      ...
      this.rosResponsesObservable$ = Rx.Observable
        .fromEventPattern(
          cb =>
            this.rosConnection.setRoscmdListener((data, running) =>
              cb([data, running])
            ),
          () => {
            this.stopCurrentExecution();
            this.rosConnection.removeRoscmdListener();
          }
        )
        .multicast(subject)
        .refCount();
    ...
    stopCurrentExecution() {
      this.rosConnection.roscmd({ cmd: 'STOP_CURRENT_EXECUTION' });
    }
  }
