          ...
          link: function (scope, element, attrs)
          {
            ...
            scope.composerSettingsToUI = function ()
            {
              if (editorToolbarService.isEnvironmentSettingsPanelActive)
              {
                ...
                if (!scope.userCameraSettings.verticalFOV) {scope.userCameraSettings.verticalFOV = 60;}
                if (!scope.userCameraSettings.nearClippingDistance) {scope.userCameraSettings.nearClippingDistance = 0.15;}
                if (!scope.userCameraSettings.farClippingDistance) {scope.userCameraSettings.farClippingDistance = 100.0;}
                if (!scope.userCameraSettings.showCameraHelper) {scope.userCameraSettings.showCameraHelper =false;}
              }
            };
