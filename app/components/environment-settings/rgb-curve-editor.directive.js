        ...
        return {
          ...
          link: function (scope, element, attrs)
          {
            ...
            scope.buildVertexList = function (channel)
            {
              ...
              if (scope.curve === undefined || scope.curve[channel] === undefined || scope.curve[channel].length < 2)
              {
                vertices = [new THREE.Vector3(0, 0, 0), new THREE.Vector3(1, 1, 0)]; // No color correction, display default curve
              }
              else
              {
                vertices = [];
            ...
            scope.updateCurveWithVertexList = function (channel, vertexList)
            {
              if (vertexList.length === 2 &&
                vertexList[0].x === 0 && vertexList[0].y === 0 &&
                vertexList[1].x === 0 && vertexList[1].y === 1)
              {
            ...
            scope.renderCurve = function ()
            {
              ...
              {
                case 0: colors.splice(2, 0, colors.splice(0, 1)[0]); break;
                case 1: colors.splice(2, 0, colors.splice(1, 1)[0]); break;
              }
            ...
            scope.$watch('selectedColorChannel', function ()
            {
            ...
            scope.mouseDown = function (e)
            {
              ...
              {
                var catmullCurve = new THREE.CatmullRomCurve3(scope.mouseEditingVertices);

                for (i = 0; i <= canvas.width && scope.draggingPointIndex === -1; i += 4)
                {
                  vec = catmullCurve.getPoint(i / canvas.width);
                  x = vec.x * canvas.width;
                  y = canvas.width - vec.y * canvas.height;
                  ...
                  if ((vx * vx + vy * vy) < (hitZone * hitZone))
                  {
                    ...
                    for (var j = 0; j < scope.mouseEditingVertices.length - 1; j += 1)
                    {
                      if (vec.x >= scope.mouseEditingVertices[j].x && vec.y < scope.mouseEditingVertices[j + 1].x)
                      {
                        var newPt = new THREE.Vector3(mx / canvas.width, 1.0 - (my / canvas.height), 0);

                        scope.draggingPointIndex = j + 1;
                        scope.mouseEditingVertices.splice(scope.draggingPointIndex, 0, newPt);
            ...
            scope.mouseMove = function (e)
            {
              ...
              if (scope.draggingPointIndex >= 0)  // Drag point
              {
                if (scope.draggingPointIndex === 0 || scope.draggingPointIndex === scope.mouseEditingVertices.length - 1)
                {
                  deltaX = 0;  // Start/end point cannot be moved on horizontal axis
                }

                var newX = scope.mouseEditingVertices[scope.draggingPointIndex].x + deltaX / canvas.width;
                var newY = scope.mouseEditingVertices[scope.draggingPointIndex].y - deltaY / canvas.height;
                ...
                scope.updateCurveWithVertexList(scope.selectedColor(), scope.mouseEditingVertices);
                scope.renderCurve();
              }
            };
