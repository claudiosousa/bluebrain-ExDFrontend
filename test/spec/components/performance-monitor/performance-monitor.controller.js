...
describe('Directive: performance-monitor', function() {
  ...
  beforeEach(inject(function(_$rootScope_, $compile, _$timeout_) {
    ...
    $scope = $rootScope.$new();
    $scope.visible = false;
    var element = $compile('<performance-monitor ng-show="visible"></performance-monitor>')($scope);
  ...
  it('should register with the performance monitor service', function() {
    expect(controller.performanceMonitorService.registerClient).toHaveBeenCalledWith(
      controller.chart
    );
  });
  ...
  it('should unregister with the performance monitor service', function() {
    ...
    $scope.$digest();
    elementScope.$digest();
    expect(controller.performanceMonitorService.unregisterClient).toHaveBeenCalledWith(
      controller.chart
    );
  });
