...
describe('Directive: show-on-top', function() {
  ...
  beforeEach(
    inject(function(_$rootScope_, $controller, _$compile_, showOnTopService) {
      $rootScope = _$rootScope_;
      $compile = _$compile_;
      MAX_PANEL_ZINDEX = Object.getPrototypeOf(showOnTopService).constructor
        .MAX_PANEL_ZINDEX;
      panels = [];
      for (var i = 0; i < 3; i++) {
        $rootScope['show' + i] = false;
        panels.push(
          $compile('<div show-on-top ng-show="show' + i + '"/>')(
            $rootScope.$new()
          )
        );
      }
