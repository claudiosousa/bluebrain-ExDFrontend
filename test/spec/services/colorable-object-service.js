...
describe('Service colorableObjectService', function() {
  ...
  it('should have an isColorableEntity function', function() {
    expect(angular.isFunction(colorableObjectService.isColorableEntity)).toBe(
      true
    );
  });
  ...
  it('should find a simple SDF object to be a colorable object', function() {
    var simpleObjectInSDFFile = buildThreeObject([
      'some_model',
      'some_link',
      SDF_SIMPLE_OBJECTS_VISUAL_NAME
    ]);
    expect(
      colorableObjectService.isColorableEntity(simpleObjectInSDFFile)
    ).toBe(true);
  });
  ...
  it('should trigger update_material request', function() {
    ...
    colorableObjectService.setEntityMaterial(
      { serverBaseUrl: CHANGE_MATERIAL_URL, simulationID: SIMULATION_ID },
      tvModel,
      MATERIAL_COLOR
    );
  ...
  it('should trigger update_material request and append emissive postfix', function() {
    ...
    httpBackend
      .expectPUT(
        ...
        {
          //eslint-disable-next-line camelcase
          visual_path: modelNames.join('::'),
          material: MATERIAL_COLOR + emissivePostfix
        }
      )
      .respond(200);
