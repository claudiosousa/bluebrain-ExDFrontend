...
describe('Services: collab-experiment-lock-service', function() {
  ...
  beforeEach(inject(function(
    ...
    spyOn(nrpUser, 'getCurrentUser').and.callFake(function() { return window.$q.when('id'); });
    ...
    lockFileRequest = httpBackend.whenGET('http://proxy/storage/FAKE_CONTEXT_ID/edit.lock?byname=true').respond(200);
  }));
  ...
  describe('createdLockServiceForContext :', function() {
    ...
    beforeEach(inject(function(_storageServer_) {
    ...
    it('tryAddLock should create a lock', function() {
      ...
      spyOn(storageServer, 'setFileContent').and.returnValue(window.$q.when({ uuid: 'fakeuuid' }));
    ...
    it('lock too old shoud be removed and ignored', function() {
      ...
      spyOn(storageServer, 'getFileContent').and.returnValue(window.$q.when({ uuid: FAKE_USER_ID, data: JSON.stringify({ userId: FAKE_USER_ID, date: moment().subtract(LOCK_FILE_VALIDITY_MAX_AGE_HOURS, 'hours') }) }));

      storageServer.deleteFile = jasmine.createSpy('deleteFile').and.returnValue($q.when());

      var lockObj;
      createdLockService = collabExperimentLockService.createLockServiceForExperimentId(FAKE_EXPERIMENT_FOLDER_ID);
      createdLockService.isLocked().then(
        function(lockObj_) {
          lockObj = lockObj_;
        });
      ...
      expect(storageServer.deleteFile).toHaveBeenCalledWith(FAKE_EXPERIMENT_FOLDER_ID, 'edit.lock', true);
    });
    ...
    it('tryAddLock should fail', function() {
      ...
      var lockinfo;
      createdLockService.tryAddLock()
        .then(function(_lockinfo) {
          lockinfo = _lockinfo;
        });
