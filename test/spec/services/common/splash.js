(function() {
  ...
  describe('Services: splash', function() {
    ...
    beforeEach(
      inject(function(_splash_, _$uibModal_) {
        splash = _splash_;
        modal = _$uibModal_;
  ...
  describe('Controller: ModalInstanceCtrl', function() {
    ...
    beforeEach(module('exdFrontendApp'));
    beforeEach(
      inject(function($rootScope, _$log_, _splash_, $controller, _$timeout_) {
        scope = $rootScope.$new();
        log = _$log_;
        splash = _splash_;
        $timeout = _$timeout_;
    ...
    it('should update the scope when receiving a well-defined message', function() {
      ...
      expect(scope.progressInformation).toEqual(
        exampleMessage.progressInformation
      );
      expect(scope.spin).toBe(true);

      var shortButWellDefinedMessageWithHeadline = {
        headline: 'some_fake_headline'
      };
      splash.spin = false;
      prepareSpiesAndSetUpTest(shortButWellDefinedMessageWithHeadline);

      expect(scope.headline).toEqual(
        shortButWellDefinedMessageWithHeadline.headline
      );
      expect(scope.subHeadline).toEqual('');
      expect(scope.progressInformation).toEqual('');
      expect(scope.spin).toBe(false);

      var shortButWellDefinedMessageWithSubHeadline = {
        subHeadline: 'some_fake_sub_headline'
      };
      prepareSpiesAndSetUpTest(shortButWellDefinedMessageWithSubHeadline);
      expect(scope.headline).toEqual('');
      expect(scope.subHeadline).toEqual(
        shortButWellDefinedMessageWithSubHeadline.subHeadline
      );
      expect(scope.progressInformation).toEqual('');
    });
